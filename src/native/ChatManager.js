 ...
 * @param ext 附件内容
 */
export const sendMessage = (conversationId, chatType, messageType, body, ext) => {
     const { from, ...oriMessageExt } = ext || {};
     let messageExt = oriMessageExt;
     const e = chatType === ChatType.group ? { conversationId, chatType } : chatType === ChatType.single && from  ? { conversationId: from, chatType } : {};
     const  extObject = {
